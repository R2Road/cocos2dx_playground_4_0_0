...
namespace algorithm_practice_floodfill
{
	...
	void ProcessorNode::setVisible( bool visible )
	{
		...
		if( !visible )
		{
			mMode = eMode::Step;
			mToolBarNode->SelectTool( static_cast<int>( mMode ) );
			unschedule( CC_SCHEDULE_SELECTOR( ProcessorNode::algorithmLoop ) );
	...
	void ProcessorNode::onModeSelect( const int mode_index )
	{
		...
		{
		case eMode::Step:
			if( isScheduled( CC_SCHEDULE_SELECTOR( ProcessorNode::algorithmLoop ) ) )
			{
				unschedule( CC_SCHEDULE_SELECTOR( ProcessorNode::algorithmLoop ) );
			}
			break;
		case eMode::Loop:
			if( !isScheduled( CC_SCHEDULE_SELECTOR( ProcessorNode::algorithmLoop ) ) )
			{
				schedule( CC_SCHEDULE_SELECTOR( ProcessorNode::algorithmLoop ) );
			}
			break;
		}
	}
