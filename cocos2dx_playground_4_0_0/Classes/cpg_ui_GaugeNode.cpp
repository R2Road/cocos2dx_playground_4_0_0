...
namespace cpg_ui
{
	...
	{
		if( !isScheduled( CC_SCHEDULE_SELECTOR( GaugeNode::update4GaugeAnimation ) ) )
		{
			schedule( CC_SCHEDULE_SELECTOR( GaugeNode::update4GaugeAnimation ) );
		}
	}
	void GaugeNode::update4GaugeAnimation( float /*delta_time*/ )
	{
		if( mCurrentRate >= mAnimationRate )
		{
			...
			unschedule( CC_SCHEDULE_SELECTOR( GaugeNode::update4GaugeAnimation ) );
		}
