...
{
	Collection06Scene::Collection06Scene( const helper::FuncSceneMover& back_to_the_previous_scene_callback ) :
	...
	Scene* Collection06Scene::create( const helper::FuncSceneMover& back_to_the_previous_scene_callback )
	{
		auto ret = new ( std::nothrow ) Collection06Scene( back_to_the_previous_scene_callback );
		if( !ret || !ret->init() )
		{
			delete ret;
			ret = nullptr;
	...
	bool Collection06Scene::init()
	{
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "bounceEase In", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::bounceEaseIn( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "bounceEase Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::bounceEaseOut( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "bounceEase In Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::bounceEaseInOut( g_x ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "ease In", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::easeIn( g_x, 2.f ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "ease Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::easeOut( g_x, 2.f ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		//
		{
			auto graph_view_node = GraphAndNameNode::create( { false, false }, "ease In Out", GraphWidth, GraphHeight, []( float g_x )->float { return tweenfunc::easeInOut( g_x, 2.f ); } );
			graph_view_node->setPosition(
				visibleOrigin
		...
		// Setup
		//
		schedule( CC_SCHEDULE_SELECTOR( Collection06Scene::update4Graph ) );

		return true;
	}

	void Collection06Scene::onEnter()
	{
		Scene::onEnter();

		assert( !mKeyboardListener );
		mKeyboardListener = EventListenerKeyboard::create();
		mKeyboardListener->onKeyPressed = CC_CALLBACK_2( Collection06Scene::onKeyPressed, this );
		getEventDispatcher()->addEventListenerWithSceneGraphPriority( mKeyboardListener, this );
	}
	void Collection06Scene::onExit()
	{
		assert( mKeyboardListener );
	...
	void Collection06Scene::update4Graph( float dt )
	{
		if( !mbPause )
		{
	...
	void Collection06Scene::updateTimeScaleView()
	{
		static_cast<Label*>( getChildByTag( TAG_TileScaleView ) )->setString( StringUtils::format( "Time Scale : %.2f", mTimeScale ) );
	}


	void Collection06Scene::onKeyPressed( EventKeyboard::KeyCode keycode, Event* /*event*/ )
	{
