...
{
	TangentScene::TangentScene( const helper::FuncSceneMover& back_to_the_previous_scene_callback ) :
	...
	Scene* TangentScene::create( const helper::FuncSceneMover& back_to_the_previous_scene_callback )
	{
		auto ret = new ( std::nothrow ) TangentScene( back_to_the_previous_scene_callback );
		if( !ret || !ret->init() )
		{
			delete ret;
			ret = nullptr;
	...
	bool TangentScene::init()
	{
		...
		//
		{
			...
			, []( float g_x )->float { return std::tan( g_x ); }
			);
		...
		// Setup
		//
		schedule( CC_SCHEDULE_SELECTOR( TangentScene::update4Graph ) );

		return true;
	}

	void TangentScene::onEnter()
	{
		Scene::onEnter();

		assert( !mKeyboardListener );
		mKeyboardListener = EventListenerKeyboard::create();
		mKeyboardListener->onKeyPressed = CC_CALLBACK_2( TangentScene::onKeyPressed, this );
		getEventDispatcher()->addEventListenerWithSceneGraphPriority( mKeyboardListener, this );
	}
	void TangentScene::onExit()
	{
		assert( mKeyboardListener );
	...
	void TangentScene::update4Graph( float dt )
	{
		if( !mbPause )
		{
	...
	void TangentScene::updateTimeScaleView()
	{
		static_cast<Label*>( getChildByTag( TAG_TileScaleView ) )->setString( StringUtils::format( "Time Scale : %.2f", mTimeScale ) );
	}


	void TangentScene::onKeyPressed( EventKeyboard::KeyCode keycode, Event* /*event*/ )
	{
