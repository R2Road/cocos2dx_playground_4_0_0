		...
		void Update( float dt ) override
		{
			...
			else
			{
				...
				label->setColor( Color3B(
					Color3B::RED.r + static_cast<uint8_t>( r * color_modify_rate )
					, Color3B::RED.g + static_cast<uint8_t>( g * color_modify_rate )
					, Color3B::RED.b + static_cast<uint8_t>( b * color_modify_rate )
				) );
			}
...
namespace step02
{
	namespace fsm1test
	{
		...
		void BasicScene::onKeyPressed( EventKeyboard::KeyCode keycode, Event* /*event*/ )
		{
			...
			{
				if( !isScheduled( CC_SCHEDULE_SELECTOR( BasicScene::updateForExit ) ) )
				{
					scheduleOnce( CC_SCHEDULE_SELECTOR( BasicScene::updateForExit ), 0.f );
				}
			}
		}
	}
