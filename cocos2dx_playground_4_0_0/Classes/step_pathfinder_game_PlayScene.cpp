...
namespace step_pathfinder
{
	namespace game
	{
		...
		bool PlayScene::init()
		{
			...
			schedule( CC_SCHEDULE_SELECTOR( PlayScene::update4PlayTime ) );
		...
		void PlayScene::GameProcess( const int move_x, const int move_y )
		{
			...
			{
				AudioEngine::play2d( "sounds/fx/coin_001.ogg", false, 0.2f );
		...
		{
			if( !isScheduled( CC_SCHEDULE_SELECTOR( PlayScene::updateForExit ) ) )
			{
				mNextSceneType = next_scene_type;
				scheduleOnce( CC_SCHEDULE_SELECTOR( PlayScene::updateForExit ), wait_time );
			}
		}
